...
import net.blerf.ftl.parser.save.drone.*;
import net.blerf.ftl.parser.save.projectile.*;
import net.blerf.ftl.parser.save.system.*;
...
public class SavedGameParser extends Parser {
	...
	public SavedGameState readSavedGame( FileInputStream in ) throws IOException {
		...
		// Always native before FTL 1.6.1.
		gameState.setRandomNative(fileFormat != 11 || readBool(in));
	...
	public void writeSavedGame( OutputStream out, SavedGameState gameState ) throws IOException {
		...
		int diffFlag;
		if ( gameState.getDifficulty() == Difficulty.NORMAL ) {
			diffFlag = 1;
		} else if (gameState.getDifficulty() == Difficulty.HARD) {
			diffFlag = 2;
		} else {
			LOGGER.warn( String.format( "Substituting EASY for unsupported difficulty for saved game: %s", gameState.getDifficulty()) );
			diffFlag = 0;
		}
	...
	private ShipState readShip( InputStream in, boolean auto, int fileFormat, boolean dlcEnabled ) throws IOException {
		...
		if ( fileFormat == 7 || fileFormat == 8 || fileFormat == 9 || fileFormat == 11 ) {
			...
			SystemState tmpSystem = shipState.getSystem(SystemType.CLONEBAY);
			...
			shieldsInfo.setShieldRaiseAnimOn( readBool( in ) );		// -1 value here sometimes?!
		...
		Map<DoorCoordinate, ShipLayoutDoor> vacuumDoorMap = new LinkedHashMap<>();
	...
	private CrewState readCrewMember( InputStream in, int fileFormat ) throws IOException {
		...
		if ( fileFormat == 7 || fileFormat == 8 || fileFormat == 9 || fileFormat == 11 ) {
			...
			if (CrewType.CRYSTAL == crew.getRace()) {
	...
	private RoomState readRoom( InputStream in, int squaresH, int squaresV, int fileFormat ) throws IOException {
		...
		if ( fileFormat == 7 || fileFormat == 8 || fileFormat == 9 || fileFormat == 11 ) {
			...
			if ( stationDirectionFlag == 0 ) {
				...
				throw new IOException( "Unsupported room station direction flag: "+ stationDirectionFlag );
			}
	...
	public void writeRoom( OutputStream out, RoomState room, int squaresH, int squaresV, int fileFormat ) throws IOException {
		...
		if ( fileFormat == 7 || fileFormat == 8 || fileFormat == 9 || fileFormat == 11 ) {
			...
			int stationDirectionFlag;
			if ( room.getStationDirection() == StationDirection.RIGHT ) {
				...
				throw new IOException( "Unsupported room station direction: "+ room.getStationDirection());
			}
	...
	public void writeEnvironment( OutputStream out, EnvironmentState env ) throws IOException {
		...
		int vulnFlag;
		if ( env.getVulnerableShips() == HazardVulnerability.NEARBY_SHIP ) {
			vulnFlag = 1;
		}
			...
			throw new IOException( String.format( "Unsupported environment vulnerability: %s", env.getVulnerableShips()) );
		}
	...
	private ProjectileState readProjectile( FileInputStream in, int fileFormat ) throws IOException {
		...
		if (ProjectileType.LASER_OR_BURST == projectile.getProjectileType()) {
			// Laser/Burst (2).
			// Usually getType() is 4 for Laser, 2 for Burst.
			extendedInfo = readLaserProjectileInfo( in );
		}
		else if (ProjectileType.ROCK_OR_EXPLOSION == projectile.getProjectileType()) {
			// Explosion/Asteroid (0).
			// getType() is always 2?
			extendedInfo = new EmptyProjectileInfo();
		}
		else if (ProjectileType.MISSILE == projectile.getProjectileType()) {
			// Missile (0).
			// getType() is always 1?
			extendedInfo = new EmptyProjectileInfo();
		}
		else if (ProjectileType.BOMB == projectile.getProjectileType()) {
			// Bomb (5).
			// getType() is always 5?
			extendedInfo = readBombProjectileInfo( in );
		}
		else if (ProjectileType.BEAM == projectile.getProjectileType()) {
			// Beam (25).
			// getType() is always 5?
			extendedInfo = readBeamProjectileInfo( in );
		}
		else if (ProjectileType.PDS == projectile.getProjectileType()) {
			// PDS (12)
			// getType() is always 5?
	...
	public void writeProjectile( OutputStream out, ProjectileState projectile, int fileFormat ) throws IOException {
		...
		else if (ProjectileType.PDS == projectile.getProjectileType() && fileFormat == 11 ) {
			projectileTypeFlag = 6;
		}
		else {
			throw new IOException( String.format( "Unsupported projectileType: %s", projectile.getProjectileType()) );
		}
		writeInt( out, projectileTypeFlag );

		if (ProjectileType.INVALID == projectile.getProjectileType()) {
	...
	public enum CrewType {
		...
		CrewType(String id, int maxHealth) {
			this.id = id;
		...
		public static CrewType findById( String id ) {
			return Arrays.stream(values()).filter(race -> race.id.equals(id)).findFirst().orElse(null);
		}
	}
	...
	public enum SystemType {
		...
		SystemType(String id, boolean subsystem) {
			this.id = id;
	...
	public enum DroneType {
		...
		public static DroneType findById( String id ) {
			return Arrays.stream(values()).filter(d -> d.id.equals(id)).findFirst().orElse(null);
		}
	}
	...
	public enum FleetPresence {
		...
		FleetPresence(String title) { this.title = title; }
	...
	private void readExtendedShipInfo( FileInputStream in, ShipState shipState, int fileFormat ) throws IOException {
		...
		for ( DroneState drone : shipState.getDroneList() ) {
			...
			if (DroneType.REPAIR == droneType ||
					DroneType.BATTLE == droneType) {
	...
	public void writeExtendedShipInfo( OutputStream out, ShipState shipState, int fileFormat ) throws IOException {
		...
		if ( hackingState != null && hackingState.getCapacity() > 0 ) {
			...
			else if (SystemType.SHIELDS == targetSystemType) targetSystemTypeFlag = 0;
			else if (SystemType.ENGINES == targetSystemType) targetSystemTypeFlag = 1;
			else if (SystemType.OXYGEN == targetSystemType) targetSystemTypeFlag = 2;
			else if (SystemType.WEAPONS == targetSystemType) targetSystemTypeFlag = 3;
			else if (SystemType.DRONE_CTRL == targetSystemType) targetSystemTypeFlag = 4;
			else if (SystemType.MEDBAY == targetSystemType) targetSystemTypeFlag = 5;
			else if (SystemType.PILOT == targetSystemType) targetSystemTypeFlag = 6;
			else if (SystemType.SENSORS == targetSystemType) targetSystemTypeFlag = 7;
			else if (SystemType.DOORS == targetSystemType) targetSystemTypeFlag = 8;
			else if (SystemType.TELEPORTER == targetSystemType) targetSystemTypeFlag = 9;
			else if (SystemType.CLOAKING == targetSystemType) targetSystemTypeFlag = 10;
			else if (SystemType.ARTILLERY == targetSystemType) targetSystemTypeFlag = 11;
			else if (SystemType.BATTERY == targetSystemType) targetSystemTypeFlag = 12;
			else if (SystemType.CLONEBAY == targetSystemType) targetSystemTypeFlag = 13;
			else if (SystemType.MIND == targetSystemType) targetSystemTypeFlag = 14;
			else if (SystemType.HACKING == targetSystemType) targetSystemTypeFlag = 15;
			else {
	...
	private DronePodState readDronePod( FileInputStream in, DroneType droneType ) throws IOException {
		...
		ExtendedDronePodInfo extendedInfo;
		if (DroneType.BOARDER == droneType) {
		...
		else if (DroneType.HACKING == droneType) {
		...
		else if (DroneType.COMBAT == droneType ||
				DroneType.BEAM == droneType) {
		...
		else if (DroneType.DEFENSE == droneType) {
			extendedInfo = new EmptyDronePodInfo();
		}
		else if (DroneType.SHIELD == droneType) {
		...
		else if (DroneType.SHIP_REPAIR == droneType) {
			ZigZagDronePodInfo zigPodInfo = new ZigZagDronePodInfo();
			zigPodInfo.setLastWaypointX( readInt( in ) );
			zigPodInfo.setLastWaypointY( readInt( in ) );
			zigPodInfo.setTransitTicks( readMinMaxedInt( in ) );
	...
	private WeaponModuleState readWeaponModule( FileInputStream in, int fileFormat ) throws IOException {
		...
		List<XYPair> currentTargetsList = new ArrayList<>();
		...
		List<XYPair> prevTargetsList = new ArrayList<>();
