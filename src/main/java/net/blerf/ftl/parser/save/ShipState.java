...
import net.blerf.ftl.parser.save.drone.DroneState;
import net.blerf.ftl.parser.save.drone.StandaloneDroneState;
import net.blerf.ftl.parser.save.system.ExtendedSystemInfo;
import net.blerf.ftl.parser.save.system.SystemState;
import net.blerf.ftl.parser.save.system.WeaponState;
...
public class ShipState {
    ...
    private Map<SavedGameParser.SystemType, List<SystemState>> systemsMap = new LinkedHashMap<>();
    ...
    private Map<DoorCoordinate, DoorState> doorMap = new LinkedHashMap<>();
    ...
    public void refit() {
        ...
        systemsMap.clear();
        int powerRequired = 0;
        for ( SavedGameParser.SystemType systemType : SavedGameParser.SystemType.values() ) {
    ...
    public void commandeer() {
        ...
        for ( Map.Entry<SavedGameParser.SystemType, List<SystemState>> entry : getSystemsMap().entrySet() ) {
     ...
     */
    public SystemState getSystem(SavedGameParser.SystemType systemType ) {
     ...
     */
    public List<SystemState> getSystems(SavedGameParser.SystemType systemType ) {
        List<SystemState> systemList = systemsMap.get( systemType );
        if ( systemList == null ) {
            systemList = new ArrayList<>(0);
    ...
    public Map<SavedGameParser.SystemType, List<SystemState>> getSystemsMap() { return systemsMap; }
    ...
    public String toString() {
        ...
        result.append( "\n" );
        first = true;
        for ( Map.Entry<SavedGameParser.SystemType, List<SystemState>> entry : systemsMap.entrySet() ) {
            for ( SystemState s : entry.getValue() ) {
                if ( first ) { first = false; }
                else { result.append( ",\n" ); }
        ...
        for (ListIterator<RoomState> it = roomList.listIterator(); it.hasNext(); ) {
            ...
            SavedGameParser.SystemType systemType = blueprintSystems.getSystemTypeByRoomId( roomId );
